...
{
    /// <summary>
    /// エディタ上での盤面プレビューを描画する（GameObject を生成せず Gizmos で表示）。
    /// </summary>
    private void OnDrawGizmos()
    {
        if (!enabled || !editorPreview || level == null) return;

        // ロジックのみ最新化（cells 等を再構築）
        ParseCellsFromLevel();

        if (cells == null) return;
        if (Height != cells.GetLength(0) || Width != cells.GetLength(1)) return;

        // 描画順：床 → 壁/アンカー/落とし穴 → 出口 → 役者マーカー
        float y0 = floorY;
        float y1 = floorY + 0.001f;
        float y2 = floorY + 0.002f;

        // 床（Exit の足元も床として扱う場合は適宜拡張）
    ...
    /// <summary>
    /// 1セル分の矩形を Gizmos で描画する。
    /// </summary>
    /// <param name="p">グリッド座標</param>
    /// <param name="y">描画Yオフセット</param>
