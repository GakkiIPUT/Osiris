...
public class GameUI : MonoBehaviour
{
    ...
    /// <summary>初期設定の一括適用（必要時のみ）。</summary>
    private void Start()
    {
        if (applyInitialOnStart) ApplyInitialSettingsOnce();
    }

    /// <summary>有効化時に参照を解決する。</summary>
    private void OnEnable()
    { ResolveRefs(); }

    /// <summary>Board/Player/Turn の参照を探索・解決する。</summary>
    ...
    /// <summary>
    /// Editor/DevBuild/本番の各プロファイルに従い、初期パラメータを1度だけ適用する。
    /// </summary>
    ...
    /// <summary>現在ステージのチュートリアル既読をリセットする。</summary>
    ...
    /// <summary>全ステージのチュートリアル既読をリセットする。</summary>
    ...
    /// <summary>
    /// 入力の一部（F1/devModeトグル、Resetキー）と回転ボタンの活性制御を行う。
    /// </summary>
    private void Update()
    {
