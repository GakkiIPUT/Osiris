...
/// <summary>
/// ASCIIマップをScene上で直接ペイント/保存できるエディタツール。
/// BoardManager と連携して即時プレビュー、.txtとの同期を提供する。
/// </summary>
public class LevelPainter : EditorWindow
{
    ...
    /// <summary>SceneView へのフックを張る。</summary>
    private void OnEnable() { SceneView.duringSceneGui += OnSceneGUI; }
    /// <summary>フックを解除する。</summary>
    private void OnDisable() { SceneView.duringSceneGui -= OnSceneGUI; }

    /// <summary>
    /// パネルGUI。ターゲット選択、ロード/セーブ、ブラシ、リサイズ、ペイントガイドなどを描画する。
    /// </summary>
    private void OnGUI()
    {
    ...
    /// <summary>
    /// 行列のサイズを変更し、右パディング'.'で揃える。即時 Board に反映。
    /// </summary>
    ...
    /// <summary>
    /// SceneView 上でハイライト・クリック配置・ショートカットを処理する。
    /// </summary>
    ...
    /// <summary>
    /// TextAsset から rows を読み込み、正規化して即時プレビューに反映。
    /// </summary>
    ...
    /// <summary>
    /// 現在の Board の level を rows に取り込み、正規化する。
    /// </summary>
    ...
    /// <summary>
    /// rows を Board に適用し、開発用のオーバーライドも保存する。
    /// </summary>
    ...
    /// <summary>テキストから行配列へ変換（空行除外）。</summary>
    ...
    /// <summary>
    /// 指定位置に記号をペイントし、即 Board に反映する。'P' は一意に保つ。
    /// </summary>
    ...
    /// <summary>マウス位置からBoard座標を取得（外周生成ON時は拡張後範囲で判定）。</summary>
    ...
    /// <summary>ブラシボタン（トグル）を描画・切替する。</summary>
